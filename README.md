...
mutate(Interest.Rate=as.numeric(gsub("%","",Interest.Rate)),
        ...
        #---- we have to convert data to numeric if it is Character.
...
mutate(el=ifelse(substr(Employment.Length,1,2)=="10",10,Employment.Length),
  ...
  #---- we have to convert data to numeric if it is Character and delete the old column as we have created dummy columns for same and we are omitting Na if they are present.
